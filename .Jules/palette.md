...
## 2026-04-13 - [Elevating Visual-Only Loading Spinners and Status Indicators]

**Learning:** Stitch-generated screens often use CSS-only or SVG-based loading spinners, skeleton loaders, or "loading..." text indicators that are completely silent and invisible to screen reader users. Dynamic state transitions need to be announced explicitly to ensure non-visual users are aware when operations are in progress. Checking element classes, IDs, and text patterns allows us to automatically assign `role="status"` and provide an `aria-label="Loading"` for empty visual icons.
**Action:** Post-process downloaded screen HTML code to identify status/loader elements using hoisted RegExp patterns, then cleanly inject `role="status"` if role, live, or busy attributes are missing, and fallback on `aria-label="Loading"` for textless spinners.
